...
class LoginController extends BaseController 
{
	...
	{
		$userid    				= $this->_getOptionalParam('userid',NULL);		
		$group_data 			= App_ModelFactory::getModel("Group")->fetchEntryByUniqueName("members");
		
		if ($userid !== NULL)
		{
			$userdata 			= App_ModelFactory::getModel("User")->fetchEntry($userid);
			
			$this->p("ok");
		
			if ($userdata['group_id'] == $group_data['id'])
			{
		...
		else
		{
			...
			$this->view->members	= App_ModelFactory::getModel("User")->fetchByGroup($group_data['id']);
		}
	}
