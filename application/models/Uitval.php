...
class App_Model_Uitval extends App_Model_Table
{ 	
	...
	function fetchByLijnnr($range,$site_id,$dienst_id)
	{
		...
		$select->from(array('u'=>'uitval'),array('gewicht'=>'SUM(gewicht_netto)','lijnnr'));
		$select->where('site_id = ?',	$site_id);
	...
	function fetchByCategoryAndLijnnr($range,$site_id)
	{
		...
		$select->from(array('u'=>'uitval'),array('gewicht'=>'SUM(gewicht_netto)','lijnnr','categorie'));
		$select->where('site_id = ?',	$site_id);
		$select->where('tijd >= ?',		$range['min']);
		$select->where('tijd < ?' ,		$range['max']);
