...
class App_InkoopPdfHelper
{
	...
	public function render($order_data,$items)
	{
		$order_data['inkooporder_total'] = "145,87";
		
		$render_data 	  = $order_data;
		
		
		$template 		  = APPLICATION_PATH."/../sites/".WEBSITE.
					  		Zend_Registry::getInstance()->paths->pdf_templates."/".
					  		Zend_Registry::getInstance()->site->inkooporder_template_name;
					  
		$config		     = APPLICATION_PATH."/../sites/".WEBSITE.
					  		Zend_Registry::getInstance()->paths->pdf_templates."/".
					  		Zend_Registry::getInstance()->site->inkooporder_config;
		...
		{
			die("InkoopPdfHelper::render(): config file of template niet gevonden.");
			return NULL;
		}	
	}
}